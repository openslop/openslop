...
- Edges come from the plugin chain. A connector plugin declares `dependencies(element)`, so one declaration drives what a job reads, what it waits for, and what makes it stale. An element carrying its own override of project state (its own reference images, say) declares no edge to that source node, so project edits neither reach it nor stale it.
...
- `lib/project/store.ts` holds per-project metadata in Zustand. Every mutation rule lives in a store action. One store is created per editor by `ProjectEditor` and supplied through `ProjectStoreProvider`, so there is no global registry to look a project up in. Components subscribe to reactive state through `useProject(selector)`; `useProjectStoreHandle()` is the escape hatch for imperative access that a render-time selector can't express — one-shot init, `.subscribe`, or reads outside render (`useAutosave`, `useMetadataSync`, `useRefineScript`). Non-React callers take the store or a state snapshot as an argument. A plugin never reaches for the store to read: it is handed the state snapshot its caller resolved inputs against, as `ctx.state`. `voice-hydrate` is the one plugin holding a store handle, because it writes.
