...
- Write the metadata_title in that same language, and the metadata_style description in English.
  ...
  - startFrame: optional. The id of an earlier <image> or <clip> whose picture this clip opens on, so consecutive shots continue from one another instead of starting fresh. Point it at the visual directly before this one when the two are the same moment. A chained clip waits for the one it continues from, so use it only where continuity matters.
