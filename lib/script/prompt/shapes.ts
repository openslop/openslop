...
export const VIDEO_SHAPES = dedent`
  ...
  Two settings on a clip decide how scenes join up; which elements you write decides the rest.

  - startFrame: what a clip opens on. "previous" continues from the end of the visual before it,
    so two shots read as one unbroken take; the clip waits for that visual and regenerates when
  ...
  - trimToDialogue: whether a visual is on screen only as long as the speech after it ("true"),
    ...
    clip's own length only decides what plays before it is cut. Untrimmed, the clip sets the pace
    and speech can only extend it.
  - loop: when the speech under a clip outruns it, whether it repeats ("true") or holds its last
    frame ("false").

  These combine into any pacing. Three shapes cover most requests; pick the closest to what the
  user asked for, and mix them scene by scene when one part of a video wants another feel:

  - Slideshow: the words lead. Narration or dialogue throughout, images with a little motion,
    clips only for hero moments, every visual trimmed to its words, startFrame "none" so each
    visual is its own cut. Explainers, stories, lessons, anything read aloud.
  - Film: the picture leads. Consecutive clips and nothing spoken, no narration, no character
    lines, untrimmed so each plays out in full, startFrame "previous" so the shots flow into
    one another. Anime, music videos, ambient scenes.
  - Motion explainer: the words lead but the picture never cuts. Narration over clips that chain
