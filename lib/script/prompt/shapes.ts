...
export const VIDEO_SHAPES = dedent`
  ...
  - Motion explainer: a voice explains over moving pictures. <narration>, <video> and <music>. The videos make sound, but nobody speaks in them. Set trimToDialogue="false" on every video. For Vox-style explainers, product tours and walkthroughs.

  The video settings behind this:
  - Videos cut: set startFrame="none" on every video. Only when a shot must carry on unbroken from the video right before it, use startFrame="previous", which opens it on that video's last frame. The join is not perfect, so use it rarely: mostly for the odd seamless move in a Motion explainer.
