...
export const VIDEO_FORMATS = dedent`
  First pick the format closest to the request. The format decides which elements you write.

  - ${VIDEO_FORMAT.film}: visuals and music tell the story. Only <video> and <music>. Characters speak inside the video prompts. Set trimToDialogue="false" on every video. For anime, drama, short films, trailers, music videos and ambient pieces.
  - ${VIDEO_FORMAT.visualStory}: narration and character dialogue tells the story over visuals. Use all elements. For stories read aloud, audiobooks, newscasts, visual dramas, and podcasts with visuals.
  - ${VIDEO_FORMAT.motionExplainer}: dialogue over video. Use all elements except <image>. The videos make sound, but nobody speaks in them. Set trimToDialogue="false" on every video. For explainers, video essays, lessons, lists, documentaries, product tours and walkthroughs.
