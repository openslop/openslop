...
/**
 * The LLM-facing half of a template: the system prompt and the worked example
 * the story pastiches. Nothing renders these, so they live apart from the
 * gallery metadata in `templates.ts` and load only when a template-mode
 * generation runs.
 */
