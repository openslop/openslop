...
/** The guidance a template gives the model, and the story it pastiches. */
export type TemplatePrompt = {
	system: string;
	exampleStory: string;
};

// An order of magnitude larger than the rest of a template and read only when a
// script is generated, so it sits apart from the catalog every page renders.
const TEMPLATE_PROMPTS: Record<string, TemplatePrompt> = {
	"pov-life": {
		system: dedent`
		# Important
	...
	"sleep-story": {
		...
		`,
		exampleStory: dedent`#Music: Soft welcoming ambient pad in C major, slow warm analog synth swells, distant felted piano notes spaced far apart, very low binaural pink noise underneath, no percussion, evokes the moment of pulling a duvet up to your chin. 50 BPM. Loopable for ~3 minutes.
...
#Narration: What's different is quieter than that. And it makes you wonder. What are you really building? And who is it for?`,
	...
	"celebrity-death": {
		system: dedent``,
		exampleStory: dedent`#Animated Image: grid of framed footballer portraits on an off-white background, thick black borders with soft drop shadows, names below each in a hand-drawn marker font. Diego Maradona in Argentina stripes, Diogo Jota in a red shirt, Pelé in Brazil's cream-and-green collar, José Antonio Reyes in red, Emiliano Sala in Nantes yellow, Gary Speed in a dark suit and tie, Suleiman al-Obeid in blue with number 10, Peter Biaksangzuala in blue-and-yellow. (Video prompt: zoom into portrait of Diego Maradona).
...
export function getTemplatePrompt(id: string): TemplatePrompt {
	const prompt = TEMPLATE_PROMPTS[id];
	if (!prompt) throw new Error(`Unknown template id "${id}"`);
	return prompt;
}
