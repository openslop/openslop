...
- Comments are a last resort, not a habit. Write one only where a choice would look odd or arbitrary to a first-time reader with no context. Anything inferable from the code itself is noise: delete comments that restate a name, a signature, or the steps directly below them. Never comment history: what broke, what the code used to be, why an earlier attempt failed. That belongs in the commit message.
