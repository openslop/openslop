...
UI and titles use **Slopella** (`--font-sans`, `.font-title`), a clean grotesque; body text and form controls — inputs, textareas, selects, dropdown menus — use **Inter** (`--font-body`). Any number that updates in place — timecodes, durations, counters, percentages — uses **`font-numeric`**, which is Slopella with its tabular figures (`tnum`) switched on, so digits hold their column without switching typeface. Never pair it with `tabular-nums`; the utility already carries it. Preformatted output (`DisclosureText`/`DisclosureJson`) uses `font-numeric` too: `whitespace-pre-wrap` keeps the whitespace, and tabular figures keep numbers in column. There is no monospace stack in the system. Marketing surfaces only: **Instrument Serif** (`--font-serif`, hero headlines) and **Sentient** (`.font-sentient`, the onboarding wordmark).
